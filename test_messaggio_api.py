#!/usr/bin/env python3
"""
Тестовый скрипт для проверки Messaggio API
"""
...
from accounts.messaggio_service import MessaggioSMSService


def test_messaggio_service():
    """Тестирует сервис Messaggio"""
    print("🧪 Тестирование Messaggio SMS Service...")
    print("=" * 50)
    
    # Создаем экземпляр сервиса
    service = MessaggioSMSService()
    ...
    print("\n🔑 Проверка API ключа:")
    if service.api_key:
        print(f"  ✅ API ключ настроен: {service.api_key[:10]}...")
    else:
        print("  ⚠️  API ключ не настроен, будет использоваться debug режим")
